...
from read_tips.serializers import GeneralModelSerializer
...
class ContentTips1ApiViewSets(viewsets.ReadOnlyModelViewSet):
    ...
    #                                                               is_activ=True)))))
    queryset = GeneralModel.objects.filter()\
        .prefetch_related(Prefetch('discount', queryset=PromotionsDiscounts.objects.filter(is_activ=True)))
