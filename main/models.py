...
class Image(SortableMixin):
    """Фотографии"""
    category = SortableForeignKey('CatalogService', on_delete=models.CASCADE, related_name='cat')
...
class Gallery(models.Model):
    ...
    def __str__(self):
        return f'{self.section.name} (номер в очереди - {self.section.position})'


class GalleryImageRelation(SortableMixin):
    """Соединение и сортировка"""
    gallery = models.ForeignKey('Gallery', verbose_name='Левая панель', on_delete=models.CASCADE)
