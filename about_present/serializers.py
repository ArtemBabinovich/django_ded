...
class AboutPresentSerializer(serializers.ModelSerializer):
    ...
    def save(self, validated_data):
        ...
        with transaction.atomic():
            presents = AboutPresent(name=validated_data['name'], email=validated_data['email'],
            ...
            presents.save()
            # создание дат календаря
            for value in validated_data['about_present']:
                Date.objects.create(date=value.get("date"), presents_id=presents.id)
        return presents
